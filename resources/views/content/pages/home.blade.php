...
@section('page-style')
@vite([ 'resources/css/calendar.css','resources/css/but.scss'])
@endsection

@section('page-script')
@vite(['resources/js/filterhome.js','resources/js/but.js','resources/js/calendar.js'])
@endsection
    ...
    <div class="row">
      <div class="col-md-6">
        <div class="category-section">
          ...
          @php
          // ترتيب الأخبار بناءً على تاريخ الإنشاء (الأحدث أولاً)
          $filteredNews = $news->filter(function($newsItem) use ($category) {
          return $newsItem->category_id == $category->id;
          })->sortByDesc('created_at')->take(4); // تأكد من ترتيبها تنازلياً
          @endphp
...
<div class="topics-container">
  ...
  <div class="categories-container">
    ...
    <div class="categories-row" style="display: flex; gap: 20px; justify-content: center; margin-bottom: 20px;">
      ...
      $filteredNews = $news->filter(function($newsItem) use ($category) {
      return $newsItem->category_id == $category->id;
      })->sortByDesc('created_at')->take(4); // ترتيب تنازلي
      @endphp
